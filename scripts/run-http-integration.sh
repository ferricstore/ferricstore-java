...
image="${FERRICSTORE_IMAGE:-quay.io/ferricstore/ferricstore:0.11.20@sha256:aeec52c27c3afb6e462f659c16b75898d9e9fd6833c8830194f7252ef4916e4f}"
...
cleanup() {
  ...
  case "$tls_dir" in
    /tmp/ferricstore-java-http-integration.*) rm -rf "$tls_dir" ;;
  esac
}
trap cleanup EXIT

for command in docker openssl curl grep mvn; do
