...
public final class SignalsExample {
    ...
    public static void main(String[] args) {
        ...
        try (FerricStoreClient client = FerricStoreClient.connect(url, new JsonCodec())) {
            Workflow review = new WorkflowClient(client).workflow("document_review", "pending_review");
            String id = "review-" + System.currentTimeMillis();

            review.state("approved", ctx -> Outcomes.complete(Map.of(
                "approvedAt", ctx.value("approvedAt"),
                "approvedBy", ctx.value("approvedBy")
            )));

            review.start(id, Map.of("documentId", "doc-1"));
            client.signal(id, "approve", "approved", null, Map.of(
                "approvedAt", Instant.now().toString(),
                "approvedBy", "user-1"
            ));

            System.out.println(review.worker("review-worker-1", List.of("approved")).runOnce());
        }
    }
}
