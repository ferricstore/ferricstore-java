...
public final class ValueRefsExample {
    private static final System.Logger LOG = System.getLogger(ValueRefsExample.class.getName());
        ...
        try (FerricStoreClient client = FerricStoreClient.connect(url, new JsonCodec())) {
            ...
            String profileRef =
                    rawRef instanceof byte[] bytes
                            ...
                            : String.valueOf(rawRef);

            Workflow account = new WorkflowClient(client).workflow("account", "hydrate");
            account.state(
                    "hydrate", ctx -> Outcomes.complete(Map.of("profile", ctx.value("profile"))));

            String id = "account-" + System.currentTimeMillis();
            client.create(
                    CreateOptions.builder(id, "account")
                            .state("hydrate")
                            .idempotent(true)
            ...
            LOG.log(
                    ...
                    "{0}",
                    account.worker("account-worker-1", List.of("hydrate")).runOnce());
        }
    }
}
